...
namespace IdentityAPI.Controllers
{
    ...
    public class IdentityController : ControllerBase
    {
        ...
        public ActionResult<dynamic> GetToken(LoginModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = db.Users.Where(x => x.UserName == model.UserName && x.Password == model.Password && x.Status == "Verified").FirstOrDefault();
                if (user != null)
                {
                    var token = GenerateToken(user);
                    return Ok(new { user.Fullname, user.Email, user.UserName, Token = token, user.Role });
                }
                else
                {
                    return Unauthorized();
